...
class Record extends Work
{
	...
	{
		if ($this->id == null) {
			$this->message = Log::logs('id', __CLASS__, Log::NOTICE);
			return false;
		}
	...
	protected function save ()
	{
		...
		if ($this->check($values)) {
			...
			if ($result > 0) {
				$this->fit($this->runtime, $this->data);
				$this->status = self::SAVE;
				Log::logs(__FUNCTION__, __CLASS__);
				return true;
			}
	...
	{
		if ($this->table == null) {
			$this->message = Log::logs('table', __CLASS__, Log::EXCEPTION);
			return false;
		}
	...
	protected function define ($column = null)
	{
		...
		if (preg_match($this->mode, $defined, $matches)) {
			### 类型检验
			$item = self::pick(self::ITEM, $matches);
			if (strpos($defined, 'UNSIGNED') !== false) {
				$item .= '+';
			}
			$result[] = self::pick($item, $this->types);
			### 其他检验
			$param = self::pick(self::PARAM, $matches);
	...
	{
		$item = strtoupper($item);
		$column = self::pick($item, $this->columns);
		if ($column != null) {
