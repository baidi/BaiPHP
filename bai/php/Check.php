...
class Check extends Work
{
	 ...
	 * </ul>
	 * 各规则可以任意组合，规则之间以空格分隔
	 * 检验规则在用户配置文件（config.php）中的Work::Check下设置
	 *
	 * @param array $setting 即时配置
	 * @return mixed false：检验通过；string：提示信息
	 */
	public function entrust ($setting = null, $source = null)
	{
	 ...
	 * @return mixed false：检验通过；string：提示信息
	 */
	protected function min ($length = 0)
	{
	...
	protected function max ($length = null, $decimal = 0)
	{
		...
		if (is_numeric($decimal) && $decimal > 0) {
			...
			### 整数部分长度
			if ($size == 1 &&
					 mb_strlen($numbers[0], $this->charset) <= $length - $decimal) {
				return false;
			}
			### 整数部分及小数部分
			if ($size == 2 && mb_strlen($numbers[1], $this->charset) <= $decimal
					&& mb_strlen($numbers[0], $this->charset) <= $length - $decimal) {
				return false;
			}
			return Log::logf(__FUNCTION__, $length, __CLASS__);
