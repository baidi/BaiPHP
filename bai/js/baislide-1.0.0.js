...
var bai = {
	...
	effect : function(item, no) {
		...
		if (no == null || isNaN(no) || no < 0)
			no = Math.round(Math.random() * item.length);
		if (no >= item.length - 1)
			return null;
	...
	add : function(row, col, accord) {
		...
		// 设置长宽、速度、一致性等私用属性值
		this.length = box.children("[style*=background]").length;
	...
	slide : function(no) {
		if (this.box == null || this.box.length == 0 || no < 0
			...
			return;
		// 当前显示项不切换
		if (this.bar.children(":eq(" + no + ")").hasClass("on"))
			return;
		// 正在切换时延后
		if (this.on) {
			if (no != null)
		...
		for ( var r = 0; r < this.row; r++) {
			for ( var c = 0; c < this.col; c++) {
				...
				piece.css({
					...
					"width" : this.pwidth,
					"height" : this.pheight,
					"background-position-x" : -c * this.pwidth,
					"background-position-y" : -r * this.pheight,
					"background-image" : fore.css("background-image"),
				...
				piece.animate({
					...
					"background-position-x" : accord ? ebx : this
							.effect(this.bx),
					"background-position-y" : accord ? eby : this
							.effect(this.by),
					"opacity" : 0
				// this.accord ? eo : 0
				}, this.speed, function() {
