<?php
global $config;
$config[Lang::LANG][Lang::ZH]['admin'] = '管理';
$config[Lang::LANG][Lang::ZH]['create'] = '新建';
$config[Lang::LANG][Lang::ZH]['update'] = '修改';
$config[Lang::LANG][Lang::ZH]['delete'] = '删除';

$config[Lang::LANG][Lang::ZH]['basin'] = '流域';
$config[Lang::LANG][Lang::ZH]['basin-name'] = '流域名：';
$config[Lang::LANG][Lang::ZH]['basin-create'] = '新建流域';
$config[Lang::LANG][Lang::ZH]['basin-update'] = '修改流域';
$config[Lang::LANG][Lang::ZH]['basin-delete'] = '删除流域';
$config[Lang::LANG][Lang::ZH]['flow'] = '流程';
$config[Lang::LANG][Lang::ZH]['flow-name'] = '流程名：';
$config[Lang::LANG][Lang::ZH]['flow-create'] = '新建流程';
$config[Lang::LANG][Lang::ZH]['flow-update'] = '修改流程';
$config[Lang::LANG][Lang::ZH]['flow-delete'] = '删除流程';
$config[Lang::LANG][Lang::ZH]['action-create'] = '新建处理';
$config[Lang::LANG][Lang::ZH]['action-delete'] = '删除处理';
$config[Lang::LANG][Lang::ZH]['page-create'] = '新建页面';
$config[Lang::LANG][Lang::ZH]['page-delete'] = '删除页面';
$config[Lang::LANG][Lang::ZH]['process'] = '处理';